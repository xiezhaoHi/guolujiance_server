...
CSkinWndHelper::CSkinWndHelper(void)
{
	...
	Image *pImage = CSkinManager::GetInstance()->GetSkinItem(_T("dialog\\icon_minimize.png"));
	m_WindowBtnImage.push_back(pImage);

	//pImage = CSkinManager::GetInstance()->GetSkinItem(_T("dialog\\dlg_maxmize_button.bmp"));
	pImage = CSkinManager::GetInstance()->GetSkinItem(_T("dialog\\icon_window.png"));
	m_WindowBtnImage.push_back(pImage);

	//pImage = CSkinManager::GetInstance()->GetSkinItem(_T("dialog\\dlg_close_btn.bmp"));
	pImage = CSkinManager::GetInstance()->GetSkinItem(_T("dialog\\icon_close.png"));
	m_WindowBtnImage.push_back(pImage);
...
// 	graphics.DrawImage(m_pBkImage,rcWindow.Width() - 50,0,m_pBkImage->GetWidth()-50,0,50,m_nTitleHeight,UnitPixel);
	...
	LinearGradientBrush lgb(Point(0, 0), Point(destRect.Width, destRect.Height), RGB(0x36,0x3f,0x45), RGB(0x36, 0x3f, 0x45));
	graphics.FillRectangle(&lgb, destRect.X, destRect.Y, destRect.Width, destRect.Height);
...
LRESULT CSkinWndHelper::OnSize(WPARAM wParam, LPARAM lParam)
{
	...
	{
		CRect rMin(rcWnd.right - 180, 15, rcWnd.right-160, 45);
		m_TitleBtn[ID_MIN_BTN].SetRect(rMin);
		CRect rMax(rcWnd.right - 112, 15, rcWnd.right-92, 45);
		m_TitleBtn[ID_MAX_BTN].SetRect(rMax);
	}

	CRect rClose(rcWnd.right - 48, 15, rcWnd.right - 28, 45);
...
// 	}
	m_bHaveCloseBtn = m_bHaveMinBox = m_bHaveMaxBox = TRUE;
	CDUIButton *pButton = AddTitleButton(rcControl,m_WindowBtnImage[2],1,ID_CLOSE_BTN);
	if (!m_bHaveCloseBtn)
	{
	...
	if (m_bHaveMaxBox||m_bHaveMinBox)
	{
		pButton = AddTitleButton(rcControl,m_WindowBtnImage[1],1,ID_MAX_BTN);
		if (!m_bHaveMaxBox)
		{
			//pButton->EnableButton(FALSE);
		}

		pButton = AddTitleButton(rcControl,m_WindowBtnImage[0],1,ID_MIN_BTN);
		if (!m_bHaveMinBox)
		{
			//pButton->EnableButton(FALSE);
		}
